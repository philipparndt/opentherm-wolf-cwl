...
// OpenTherm Data-ID names (standard + ventilation/heat-recovery)
...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintf(os.Stderr, "Usage: %s <analog.csv>\n", os.Args[0])
		os.Exit(1)
	}

	samples, err := loadCSV(os.Args[1])
	if err != nil {
	...
	for i, pkt := range packets {
		...
		// Extract OpenTherm frame: start(1) + data(32) + stop(1)
		startBit := rawBits[0]
		stopBit := rawBits[33]
		dataBits := rawBits[1:33]

		// Build 32-bit data word
		var frame uint32
		...
		// Parity check (even parity over all 32 bits)
		ones := bits.OnesCount32(frame)
		parityOK := ones%2 == 0

		// Float interpretation (f8.8 format used by OpenTherm)
...
func loadCSV(path string) ([]Sample, error) {
	...
	for {
		...
		samples = append(samples, Sample{Time: t, Voltage: v})
	}
	return samples, nil
}
...
func decodeManchester(pkt Packet) []int {
	...
	var bits []int
	atMidBit := false // first edge is boundary
